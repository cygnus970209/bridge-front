<template>
    <div class="input">
        <label class="input__box">
            <h4 v-if="props.title" :class="['input__title', props.isRequired ? 'input__title--required' : '']">{{ props.title }}</h4>
            <div :class="['input__inner', props.isDisabled ? 'input__inner--disabled' : '']">
                <div :class="['input__item', isFocus ? 'input__item--focus' : '']">
                    <input :type="props.type" :placeholder="props.placeholder" :value="props.value" :maxlength="props.maxlength"
						:disabled="props.isDisabled" :readonly="props.isReadonly" autocomplete="new-password"
						@input="checkLength" @focus="focusFunction" @blur="blurFunction" class="input__field"> 
                    <span v-if="props.isComplete && props.showCheckIcon" class="input__icon"></span>
                </div>
				<AtomBaseButton
					v-if="props.btnText"
					:isDisabled="props.isDisabled || props.isDisabledBtn"
